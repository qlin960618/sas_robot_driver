...
class RobotDriverProvider:
    ...
    def __init__(self, node_prefix):
        ...
        self.publisher_joint_states_ = rospy.Publisher(node_prefix + "set/joint_states",
                                                       Float64MultiArray,
                                                       queue_size=1)

        self.publisher_joint_limits_min_ = rospy.Publisher(node_prefix + "set/joint_limits_min",
                                                           Float64MultiArray,
                                                           queue_size=1)

        self.publisher_joint_limits_max_ = rospy.Publisher(node_prefix + "set/joint_limits_max",
                                                           Float64MultiArray,
                                                           queue_size=1)

        self.publisher_reference_frame_ = rospy.Publisher(node_prefix + "set/reference_frame",
                                                          PoseStamped,
                                                          queue_size=1)
        self.subscriber_target_joint_positions_ = rospy.Subscriber(node_prefix + "get/target_joint_positions",
                                                                   Float64MultiArray,
